...
exports.registerUser = async (req, res) => {
    ...
    db.query(
        'INSERT INTO users (username, name, email, password, role, phone, address) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
exports.loginUser = (req, res) => {
    ...
    db.query('SELECT * FROM users WHERE email = ? OR username = ?', [usernameOrEmail, usernameOrEmail], async (err, results) => {
        ...
        res.json({ 
            ...
            token, 
            role: user.role, 
            user: { id: user.id, name: user.name, email: user.email, username: user.username, phone: user.phone, address: user.address }
        });
    });
};

exports.getUsers = (req, res) => {
    db.query('SELECT id, username, name, email, role, phone, address FROM users', (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(results);
    });
};
